...
exports.detectActivity = detectActivity;
...
exports.parseDPRText = parseDPRText;
const ELEMENT_CANONICAL = {
    footing: 'FOOTING',
    pier: 'PIER',
    abutment: 'ABUTMENT',
    deck: 'DECK',
...
function fuzzyPick(tokens, candidates, maxDistance = 1) {
...
function detectActivity(text) {
    ...
    const tokens = normalized.split(' ');
    for (const token of tokens) {
        if (ACTIVITY_CANONICAL[token])
            return ACTIVITY_CANONICAL[token];
    }
...
function computeQuantity(dimensions) {
...
function buildFinalJSON(parsed, _text) {
